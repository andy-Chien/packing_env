...
def main():
    ...
    box_bound = [[0.5, 0.5, 0.5], [0.6, 0.6, 0.6]]
    ...
    camera_list = []
    if camera_manager.create_camera('cam_1', 'Bullet_Camera', 'OBJ_CAM', \
                                    [0, 0.3, 1.35], [0, 0, 1.25], [0, -0.1, 0.3]):
        camera_list.append('cam_1')

    if camera_manager.create_camera('cam_2', 'Bullet_Camera', 'OBJ_CAM', \
                                    [0, -0.3, 1.35], [0, 0, 1.25], [0, 0.1, 0.3]):
        camera_list.append('cam_2')

    for eps in range(10):
        model_list = model_manager.sample_models_in_bound(box_bound, 0.8, min_size_rate=0.001, generate_box=False)
